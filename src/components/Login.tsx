...
export default function Login() {
    ...
    async function handleLogin(e: React.FormEvent) {
        ...
        const user = await loginService.login(email.value, password.value);
        ...
        <div className="size-full flex items-center justify-center bg-slate-100 ">
            <div className="bg-white rounded-2xl p-4 w-1/3">
                ...
                <form onSubmit={handleLogin} className="flex flex-col gap-2">
                    <Input value={email.value} onChange={email.onChange} placeholder="your@email.com" id="email" />
                    <Input value={password.value} onChange={password.onChange} placeholder="**********" type="password" name="password" id="password" />
